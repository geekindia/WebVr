if ( ! Detector.webgl ) Detector.addGetWebGLMessage();
			...
			function rotateCar(data){
				//camera.position.x = Math.cos( data.clientX );
				//camera.position.y += ( - data.clientY - camera.position.y ) * .05;
				//camera.position.z = Math.sin( data.clientY );
				camera.position.x = 0;
				camera.position.y = 400;
				camera.position.z = 1600;
				group.rotation.y = -(data.clientZ * Math.PI / 180);
			}
